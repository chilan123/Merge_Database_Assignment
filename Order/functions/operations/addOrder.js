...
module.exports = async function addOrder(req, res) {
    ...
    if (!order_id || !order_qty || !userId || !inventoryId) {
        res.status(400).json({ Notification: 'Missing required data' });
        return;
    }

    //Check item and quantity arrays's lenths are equal 
    if (order_qty.length != inventoryId.length) return res.json('some data make NULL & Item and quantity mismatched');
